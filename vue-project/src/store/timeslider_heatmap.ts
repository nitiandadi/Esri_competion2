...
export const useTimesliderStore = defineStore('timerslider', () => {
    ...
    function watchHeatmap(){
        toRaw(timeSlider.value)?.watch('timeExtent', (timeExtent) => {
            
            view.map.allLayers.forEach(function(index){
                if(index.type === 'feature'){
                    
                    if((index as __esri.FeatureLayer).renderer.type === 'heatmap'){
                        if((index as __esri.FeatureLayer).timeExtent.start.getTime() === timeExtent.start.getTime()){
                            index.visible = true;
                        }else{
                            index.visible = false;
                        }                                                                          
    ...
    return { createTimeslider,timeSlider,watchHeatmap}
})
