    ...
    <div ref="screenRef" class="screen">
        <transition name="right-slide-in">
            <el-card class="box-card" :style="{ left: showCard ? '0.5%' : '-380px' }">
                ...
                <el-row>
                    <el-col>
                        <el-text>查看景点建筑</el-text>
                        <el-switch v-model="switchList[0].value" size="large" class="mt-2" style="margin-left: 24px"
                            inline-prompt :active-icon="Check" :inactive-icon="Close" @change="checkSwitch(0)" />
                    </el-col>
                    <el-col>
                        <el-text>管理游客信息</el-text>
                        <el-switch v-model="switchList[1].value" size="large" class="mt-2" style="margin-left: 24px"
                            inline-prompt :active-icon="Check" :inactive-icon="Close" @change="checkSwitch(1)" />
                    </el-col>
                    <el-col>
                        <el-text>游客类型分类</el-text>
                        <el-switch v-model="switchList[2].value" size="large" class="mt-2" style="margin-left: 24px"
                            inline-prompt :active-icon="Check" :inactive-icon="Close" @change="checkSwitch(2)" />
                    </el-col>
                </el-row>
            </el-card>
        </transition>
        <buildingInfro v-if="switchList[0].value" />
    </div>
</template>
