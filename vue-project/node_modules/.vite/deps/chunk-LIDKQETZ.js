...
function f2(i2, n) {
  ...
  if (s(f3))
    return y2(f3, g2, v2);
  if (l(f3))
    return f3.points = f3.points.map((t2) => y2(t2, g2, v2)), f3;
  if (u(f3))
    return d(f3, p3);
  if (y(f3) || f(f3)) {
...
function I(t2, i2) {
  ...
  if (r2 > 2 * m) {
    ...
    for (let s4 = _ + 1; s4 < c2; s4++)
      p3.push(s4), f3.push(s4);
    s3.push(new C(t3, [_]), new C(i3, [c2]), new C(r3, p3), new C(a3, f3));
  } else
    x > l2 || a2 ? s3.push(new C({ xmin: x, ymin: n, xmax: m, ymax: e }, [_]), new C({ xmin: u3, ymin: n, xmax: l2, ymax: e }, [c2])) : s3.push(new C({ xmin: x, ymin: n, xmax: l2, ymax: e }, [_]));
  return s3;
}
function g(t2, i2) {
...
var T = class {
  ...
  cut(t2, i2) {
    ...
    if (this._xCut = i2, t2.rings)
      this._closed = true, s3 = t2.rings, this._minPts = 4;
    else {
      if (!t2.paths)
        return null;
...
//# sourceMappingURL=chunk-LIDKQETZ.js.map
