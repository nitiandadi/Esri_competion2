...
var o = class {
  ...
  destroy() {
    ...
    for (const e5 in this._properties) {
      const t5 = this._properties[e5];
      for (const e6 of t5.acquired)
        x(e6) || t5.pool.release(e6);
      t5.pool.destroy(), t5.pool = null, t5.acquired = null;
    }
    this._properties = null, this._owner = null;
  }
  get(e5) {
    const t5 = this._owner._get(e5), r = this._properties[e5];
  ...
  _release() {
    for (const e5 in this._properties) {
      ...
      let o3 = 0;
      for (const e6 of t5.acquired)
        x(e6) ? t5.acquired[o3++] = e6 : t5.pool.release(e6);
...
var e4 = class {
  constructor(e5, t5 = []) {
    this.eventType = e5, this.keyModifiers = t5;
  }
  matches(e5) {
...
var i2 = class {
  ...
  registerIncoming(e5, n2, i4) {
    ...
    const o3 = "string" == typeof e5 ? new e4(e5, a4) : e5, h = () => {
      ...
      const t5 = this._incoming[e6.match.eventType];
      if (t5) {
        const n3 = t5.indexOf(e6);
        t5.splice(n3, 1), h(), this._manager && this._manager.updateDependencies();
      }
    }, g2 = new s2(o3, i4, { onPause: r, onRemove: r, onResume: (e6) => {
      const t5 = this._incoming[e6.match.eventType];
      t5 && !t5.includes(e6) && (t5.push(e6), h(), this._manager && this._manager.updateDependencies());
    } });
  ...
  registerOutgoing(e5) {
    ...
    const t5 = new a3(e5, { onEmit: (e6, t6, n2, i4) => {
      var _a;
      (_a = this._manager) == null ? void 0 : _a.emit(e6.eventType, t6, n2, i4);
    }, onRemove: (e6) => {
      var _a;
      delete this._outgoing[e6.eventType], (_a = this._manager) == null ? void 0 : _a.updateDependencies();
    } });
    return this._outgoing[e5] = t5, this._outgoingEventTypes = null, this._manager && this._manager.updateDependencies(), t5;
  }
  ...
  _handleEvent(e5) {
    var _a;
    const t5 = this._incoming[e5.type];
    if (t5) {
      for (const n2 of t5)
        if (n2.match.matches(e5) && ((_a = n2.callback) == null ? void 0 : _a.call(n2, e5), e5.shouldStopPropagation()))
          break;
    }
  }
};
var s2 = class {
  constructor(e5, t5, n2) {
    this.match = e5, this._callback = t5, this._handler = n2;
  }
  pause() {
...
var a3 = class {
  constructor(e5, t5) {
    this.eventType = e5, this._removed = false, this._handler = t5;
  }
  emit(e5, t5, n2) {
    this._removed || this._handler.onEmit(this, e5, t5, n2);
  }
  remove() {
...
var s3 = class extends i2 {
  ...
  _update(t5) {
    const s4 = "touch" === t5.native.pointerType ? "touch" : "mouse", { x: e5, y: i4 } = t5;
...
var u = class extends v {
  ...
  installHandlers(e5, t5, i4 = P.INTERNAL) {
    ...
    const n2 = { name: e5, handlers: t5.map((e6) => ({ handler: e6, active: true, removed: false, priorityIndex: 0, groupPriority: i4, eventCallback: null, uninstallCallback: null })) };
    ...
    for (let r = n2.handlers.length - 1; r >= 0; r--) {
      ...
      this._handlers.push(e6), e6.handler.onInstall({ updateDependencies: () => {
        ...
        this._emitInputEvent(e6.priorityIndex + 1, t6, r2, i5, s4, n3);
      }, setPointerCapture: (t6, r2) => {
        this._setPointerCapture(n2, e6, t6, r2);
      }, setEventCallback: (t6) => {
        e6.eventCallback = t6;
      }, setUninstallCallback: (t6) => {
        e6.uninstallCallback = t6;
      }, refreshHasPendingInputs: () => {
  ...
  uninstallHandlers(e5) {
    const t5 = this._nameToGroup[e5];
    t5 ? (t5.handlers.forEach((e6) => {
      var _a;
  ...
  updateDependencies() {
    ...
    const e5 = /* @__PURE__ */ new Set(), t5 = /* @__PURE__ */ new Set();
    ...
    this._sourceEvents = e5, this._keyModifiers = t5, this._pointerCaptures.size > 0 && this._sourceEvents.add("pointer-capture-lost"), this._keyModifiers.size > 0 && (this._sourceEvents.add("key-down"), this._sourceEvents.add("key-up")), this.eventSource && (this.eventSource.activeEvents = this._sourceEvents);
  }
  _setLatestPointer(e5, t5, r) {
    ...
    if (t(n2) || n2.x !== t5 || n2.y !== r) {
      ...
      e6.x = t5, e6.y = r, this._set("latestPointerLocation", e6);
    }
  }
  _onEventReceived(e5, t5) {
    if ("pointer-capture-lost" === e5) {
      const e6 = t5;
      this._pointerCaptures.delete(e6.native.pointerId);
    }
    this._updateKeyModifiers(e5, t5);
    ...
    this._emitInputEventFromSource(e5, t5, r, i4);
  }
  _updateKeyModifiers(e5, t5) {
    if (!t5)
      return;
    let r = false;
    const i4 = () => {
      ...
      t6 && !this._activeKeyModifiers.has(e6) ? (i4(), this._activeKeyModifiers.add(e6)) : !t6 && this._activeKeyModifiers.has(e6) && (i4(), this._activeKeyModifiers.delete(e6));
    };
  ...
  _installRecognizers() {
    this._latestPointerHandler = new s3((e5, t5, r) => this._setLatestPointer(e5, t5, r)), this._multiTouchHandler = new i3(), this.installHandlers("input-manager-logic", [this._latestPointerHandler, this._multiTouchHandler], P.ALWAYS), this.recognizers.length > 0 && this.installHandlers("default", this.recognizers, P.INTERNAL);
  }
  _setPointerCapture(e5, t5, r, i4) {
    const n2 = e5.name + "-" + t5.priorityIndex, s4 = this._pointerCaptures.get(r.pointerId) || /* @__PURE__ */ new Set();
  ...
  _emitInputEventFromSource(e5, t5, r, i4) {
    this._emitInputEvent(0, e5, t5, r, i4);
  }
  _emitInputEvent(e5, t5, r, i4, n2, s4) {
    const o3 = void 0 !== i4 ? i4 : this._currentPropagation ? this._currentPropagation.timestamp : performance.now(), a4 = void 0 !== n2 && n2, l = { event: new _(t5, r, o3, s4 || this._activeKeyModifiers, a4), priorityIndex: e5 };
  ...
  _continuePropagation() {
    ...
    for (; e5.events.length > 0; ) {
      const { event: t5, priorityIndex: r } = e5.events.pop(), i4 = t5.data && t5.data.eventId;
      if (!(null != i4 && this._stoppedPropagationEventIds.has(i4)))
        for (e5.currentHandler = this._handlersPriority[r]; e5.currentHandler; ) {
          if (e5.currentHandler.removed)
            ...
            if (e5.currentHandler.active && !t5.shouldStopPropagation() && ((_b = (_a = e5.currentHandler).eventCallback) == null ? void 0 : _b.call(_a, t5)), t5.shouldStopPropagation()) {
              null != i4 && this._stoppedPropagationEventIds.add(i4);
              break;
            }
            if (t5.shouldPausePropagation(() => this._continuePropagation()))
              return void this._pausePropagation({ event: t5, priorityIndex: e5.currentHandler.priorityIndex + 1 });
          }
  ...
  _compareHandlerPriority(e5, t5) {
    if (e5.handler.hasSideEffects !== t5.handler.hasSideEffects)
      return e5.handler.hasSideEffects ? 1 : -1;
    if (e5.groupPriority !== t5.groupPriority)
      return e5.groupPriority > t5.groupPriority ? -1 : 1;
    for (const r of e5.handler.incomingEventMatches)
      for (const e6 of t5.handler.incomingEventMatches) {
        if (r.eventType !== e6.eventType)
          continue;
        const t6 = r.keyModifiers.filter((t7) => e6.keyModifiers.includes(t7));
        if (t6.length === r.keyModifiers.length !== (t6.length === e6.keyModifiers.length))
          return r.keyModifiers.length > e6.keyModifiers.length ? -1 : 1;
      }
    return e5.priorityIndex > t5.priorityIndex ? -1 : 1;
  }
  _sortHandlersPriority(e5) {
    const t5 = [];
    for (const r of e5) {
      let e6 = 0;
      for (; e6 < t5.length && this._compareHandlerPriority(r, t5[e6]) >= 0; )
        e6++;
      t5.splice(e6, 0, r);
    }
    return t5;
...
var _ = class {
  constructor(e5, t5, r, i4, n2) {
    this.type = e5, this.data = t5, this.timestamp = r, this.modifiers = i4, this.cancelable = n2, this._propagationState = g.NONE, this._resumeCallback = null;
  }
  stopPropagation() {
  ...
  async(e5) {
    ...
    return ("function" == typeof e5 ? e5() : e5).then((e6) => t5(e6, false), (e6) => t5(e6, true));
  }
...
//# sourceMappingURL=chunk-GUVJ2XOH.js.map
