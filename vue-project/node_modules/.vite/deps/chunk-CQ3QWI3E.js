...
var R = (e, t2, n, o = 0) => {
  ...
  for (let g = 0; g < e.length; g += 2)
    isNaN(e[g]) || (i = i > e[g] ? e[g] : i, r2 = r2 > e[g] ? r2 : e[g], s3 = s3 > e[g + 1] ? e[g + 1] : s3, a2 = a2 > e[g + 1] ? a2 : e[g + 1]);
...
function O2(e) {
  ...
  if (u && f2.length !== u.length) {
    ...
    for (let e2 = 0, t3 = 0; e2 < u.length; e2++)
      u[e2] ? m.push(f2[t3++]) : m.push([NaN, NaN]);
  }
  return m;
}
function I(e) {
...
function B(e, t2 = 0, n = false) {
  ...
  if (!s3) {
    if (s3 = i.find((e2) => 1 === e2.getInclusive() && 0 === e2.getKind()), !s3)
      return null;
    r2 = true;
  }
  const a2 = n ? 0 : (2 === o.getNorthPoleLocation() ? 1 : 0) | (2 === o.getSouthPoleLocation() ? 2 : 0), c = o.isPannableRectangle(), f2 = s3.getCoord();
  if (r2)
    return { isEnvelope: r2, isPannable: c, vertices: f2, coef: null, bbox: [f2[0][0] - t2, f2[0][1] - t2, f2[1][0] + t2, f2[1][1] + t2], poleLocation: a2 };
  let u = 0;
  const m = [];
  ...
  for (let t2 = 0; t2 < r2; t2++) {
    ...
    if (n2.length > 2) {
      ...
      for (let r3 = 1; r3 < o2.length; r3++)
        2 === o2[r3] && r3 !== o2.length - 1 || (o2[r3] !== e3 && (i2.push(0 === e3 ? Math.min(t3, n2[r3 - 1]) : Math.max(t3, n2[r3 - 1])), e3 = o2[r3], t3 = n2[r3]), r3 === o2.length - 1 && i2.push(0 === o2[r3] ? Math.min(t3, n2[r3]) : Math.max(t3, n2[r3])));
      i2.sort((e4, t4) => e4 - t4);
    } else
...
function X(e, n) {
  ...
  if (2 !== f2.rings.length || !f2.rings[0].length || !f2.rings[1].length)
    return null;
  ...
  for (let t2 = 0; t2 < 2; t2++) {
    ...
    for (let e2 = 0; e2 < u[t2].length; e2++)
      i = i > u[t2][e2][0] ? u[t2][e2][0] : i, s3 = s3 < u[t2][e2][0] ? u[t2][e2][0] : s3, r2 = r2 > u[t2][e2][1] ? u[t2][e2][1] : r2, a2 = a2 < u[t2][e2][1] ? u[t2][e2][1] : a2;
    if (0 === t2)
      p.ymin = r2, p.ymax = a2, p.xmin = i, p.xmax = s3;
    else if (p.ymin = Math.min(p.ymin, r2), p.ymax = Math.max(p.ymax, a2), Math.abs(s3 - o / 2) < m)
...
function $2(e) {
  ...
  if (i.error = a2, o[0] > 1 && (i.coefficients = te(i.offsets, r2, s3)), e.includeGCSGrid && !c.isGeographic && !c.isWebMercator)
    if (f2.isGeographic)
...
function ee(e) {
  ...
  for (let n = 0; n < P3; n++) {
    ...
    for (let t3 = 0; t3 < G2; t3++) {
      let o2 = N2[n * G2 + t3];
      if (g && o2[0] > z2 && o2[0] > x / 2 - B2 ? o2[0] -= x : g && 0 === n && o2[0] < 0 && F2 && !s3 && (o2[0] += x), !o2 || isNaN(o2[0]) || isNaN(o2[1]))
        E3.push(C2), E3.push(C2), e2.push(null), v3++;
      else {
        if (s3) {
  ...
  for (let l = 0; l < o - 1; l++) {
    for (let t3 = 0; t3 < i - 1; t3++) {
      ...
      for (let e2 = 0; e2 < 3; e2++)
        r2[d2++] = s3[y++] * c + s3[y++] * u + s3[y++] * p;
      y = 0;
      for (let e2 = 0; e2 < 3; e2++)
        r2[d2++] = s3[y++] * f2 + s3[y++] * m + s3[y++] * g;
      y = 0;
      for (let e2 = 0; e2 < 3; e2++)
        r2[d2++] = a2[y++] * c + a2[y++] * h + a2[y++] * p;
      y = 0;
      for (let e2 = 0; e2 < 3; e2++)
        r2[d2++] = a2[y++] * f2 + a2[y++] * x + a2[y++] * g;
    }
    if (n)
...
//# sourceMappingURL=chunk-CQ3QWI3E.js.map
