import {
  t
} from "./chunk-HVCAHUYB.js";

// node_modules/@arcgis/core/core/RandomLCG.js
var t2 = class {
  constructor(t4 = 1) {
    this._seed = t4;
  }
  set seed(e2) {
    this._seed = e2 ?? Math.random() * t2._m;
  }
  getInt() {
    return this._seed = (t2._a * this._seed + t2._c) % t2._m, this._seed;
  }
  getFloat() {
    return this.getInt() / (t2._m - 1);
  }
  getIntRange(t4, e2) {
    return Math.round(this.getFloatRange(t4, e2));
  }
  getFloatRange(e2, s3) {
    const n2 = s3 - e2;
    return e2 + this.getInt() / t2._m * n2;
  }
};
t2._m = 2147483647, t2._a = 48271, t2._c = 0;

// node_modules/@arcgis/core/core/arrayUtils.js
function u(t4, n2) {
  return n2 ? t4.filter((t5, e2, r2) => r2.findIndex(n2.bind(null, t5)) === e2) : t4.filter((t5, n3, e2) => e2.indexOf(t5) === n3);
}
function i(n2, e2, r2) {
  if (t(n2) && t(e2))
    return true;
  if (t(n2) || t(e2) || n2.length !== e2.length)
    return false;
  if (r2) {
    for (let t4 = 0; t4 < n2.length; t4++)
      if (!r2(n2[t4], e2[t4]))
        return false;
  } else
    for (let t4 = 0; t4 < n2.length; t4++)
      if (n2[t4] !== e2[t4])
        return false;
  return true;
}
function c(t4, n2) {
  let e2 = t4.length !== n2.length;
  e2 && (t4.length = n2.length);
  for (let r2 = 0; r2 < n2.length; ++r2)
    t4[r2] !== n2[r2] && (t4[r2] = n2[r2], e2 = true);
  return e2;
}
function s(t4, n2, e2) {
  let r2, o2;
  return e2 ? (r2 = n2.filter((n3) => !t4.some((t5) => e2(t5, n3))), o2 = t4.filter((t5) => !n2.some((n3) => e2(n3, t5)))) : (r2 = n2.filter((n3) => !t4.includes(n3)), o2 = t4.filter((t5) => !n2.includes(t5))), { added: r2, removed: o2 };
}
function a(t4) {
  return t4 && "number" == typeof t4.length;
}
var d = !!Array.prototype.fill;
function m(t4, n2) {
  if (d)
    return new Array(t4).fill(n2);
  const e2 = new Array(t4);
  for (let r2 = 0; r2 < t4; r2++)
    e2[r2] = n2;
  return e2;
}
function p(t4, n2) {
  void 0 === n2 && (n2 = t4, t4 = 0);
  const e2 = new Array(n2 - t4);
  for (let r2 = t4; r2 < n2; r2++)
    e2[r2 - t4] = r2;
  return e2;
}
var x = class {
  constructor() {
    this.last = 0;
  }
};
var y = new x();
function b(t4, n2, e2, r2) {
  r2 = r2 || y;
  const o2 = Math.max(0, r2.last - 10);
  for (let f2 = o2; f2 < e2; ++f2)
    if (t4[f2] === n2)
      return r2.last = f2, f2;
  const l = Math.min(o2, e2);
  for (let f2 = 0; f2 < l; ++f2)
    if (t4[f2] === n2)
      return r2.last = f2, f2;
  return -1;
}
function v(t4, n2, e2, r2) {
  const o2 = e2 ?? t4.length, l = b(t4, n2, o2, r2);
  if (-1 !== l)
    return t4[l] = t4[o2 - 1], null == e2 && t4.pop(), n2;
}
var A = /* @__PURE__ */ new Set();
function j(t4, n2, e2 = t4.length, r2 = n2.length, o2, l) {
  if (0 === r2 || 0 === e2)
    return e2;
  A.clear();
  for (let u3 = 0; u3 < r2; ++u3)
    A.add(n2[u3]);
  o2 = o2 || y;
  const f2 = Math.max(0, o2.last - 10);
  for (let u3 = f2; u3 < e2; ++u3)
    if (A.has(t4[u3]) && (l && l.push(t4[u3]), A.delete(t4[u3]), t4[u3] = t4[e2 - 1], --e2, --u3, 0 === A.size || 0 === e2))
      return A.clear(), e2;
  for (let u3 = 0; u3 < f2; ++u3)
    if (A.has(t4[u3]) && (l && l.push(t4[u3]), A.delete(t4[u3]), t4[u3] = t4[e2 - 1], --e2, --u3, 0 === A.size || 0 === e2))
      return A.clear(), e2;
  return A.clear(), e2;
}
var C = new t2();
function F(t4, n2) {
  const e2 = t4.indexOf(n2);
  return -1 !== e2 ? (t4.splice(e2, 1), n2) : null;
}

// node_modules/@arcgis/core/core/typedArrayUtil.js
function t3(r2) {
  return r2 instanceof ArrayBuffer;
}
function n(r2) {
  return r2 && r2.constructor && "Int8Array" === r2.constructor.name;
}
function o(r2) {
  return r2 && r2.constructor && "Uint8Array" === r2.constructor.name;
}
function c2(r2) {
  return r2 && r2.constructor && "Uint8ClampedArray" === r2.constructor.name;
}
function u2(r2) {
  return r2 && r2.constructor && "Int16Array" === r2.constructor.name;
}
function e(r2) {
  return r2 && r2.constructor && "Uint16Array" === r2.constructor.name;
}
function a2(r2) {
  return r2 && r2.constructor && "Int32Array" === r2.constructor.name;
}
function s2(r2) {
  return r2 && r2.constructor && "Uint32Array" === r2.constructor.name;
}
function f(r2) {
  return r2 && r2.constructor && "Float32Array" === r2.constructor.name;
}
function i2(r2) {
  return r2 && r2.constructor && "Float64Array" === r2.constructor.name;
}
var y2 = 1024;

export {
  t2 as t,
  u,
  i,
  c,
  s,
  a,
  m,
  p,
  x,
  b,
  v,
  j,
  F,
  t3 as t2,
  n,
  o,
  c2,
  u2,
  e,
  a2,
  s2,
  f,
  i2,
  y2 as y
};
//# sourceMappingURL=chunk-MKHZ5X6F.js.map
